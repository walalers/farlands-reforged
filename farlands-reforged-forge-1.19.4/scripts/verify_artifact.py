...
Beyond the usual metadata checks this guards the things a successful build happily gets wrong:
  ...
  * Forge 1.19 - 1.20.4 runs on **SRG** names - the opposite of 1.20.6 and later. Its installer renames
...
with zipfile.ZipFile(jar) as zf:
    ...
    if refmap_name not in names:
        raise SystemExit(f'{refmap_name} is missing; Forge 1.19.x runs on SRG and the mixins would find '
...
expected = [
    ...
    # Not JAVA_{java_version}: every Forge 1.19.x build bundles Mixin 0.8.5, whose CompatibilityLevel
...
# 5. Mixin 0.8.5, which every Forge 1.19.x build bundles, rejects a static @Redirect handler inside an
...
for mixin in ('BlendedNoiseMixin', 'ImprovedNoiseMixin'):
    ...
    if re.search(r'\bstatic\b[^\n]*farlandsreforged\$', listing):
        raise SystemExit(f'{mixin} has a static handler; Mixin 0.8.5 on Forge 1.19.x rejects it.')
