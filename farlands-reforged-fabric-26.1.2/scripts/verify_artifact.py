...
jar = Path(sys.argv[1]) if len(sys.argv) > 1 else Path('build/libs/farlandsreforged-0.3.1+mc26.1.2-fabric.jar')
required_entries = {
    'fabric.mod.json',
...
print(f'OK: {jar} contains Fabric 26.1.2 metadata, config/command/event classes, advancement, lang, and mixin config.')
